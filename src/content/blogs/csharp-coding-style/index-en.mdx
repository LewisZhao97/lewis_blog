...
description: This post defines naming conventions and formatting standards for C# code within the project. This specification aims to unify the team's coding style and improve code readability and maintainability.
...
<Aside type='note'>
  **The code naming conventions in this article are for reference only. If you have your own personal or team naming conventions, please stick to them!**
</Aside>

---
## Naming convention

| Category   | Types                | Rule       | Examples       |
| :-------------- | :----------------------------- | :---------------- | :----------------- |
| **Fields** | `[SerializeField]` / `private` | `m_` + PascalCase | `m_PlayerHealth`   |
|                 | `protected`                    | `m_` + PascalCase | `m_ArmorValue`     |
|                 | `public`                       | PascalCase        | `PlayerScore`      |
| **Static** | `private static`               | `s_` + PascalCase | `s_Instance`       |
|                 | `public static`                | PascalCase        | `Instance`         |
| **Const**  | `const`                        | `k_` + PascalCase | `k_MaxSpeed`       |
| **Props**  | `public` / `private`           | PascalCase        | `Health`           |
| **Types**  | `class` / `struct` / `enum`    | PascalCase        | `PlayerController` |
|                 | `interface`                    | `I` + PascalCase  | `IDamageable`      |
| **Others**          | `method`                   | PascalCase        | `GetPosition()`    |
|                 | `event`                   | PascalCase        | `OnLevelStart`     |

---
## Code Format

To keep the code clean, we follow these formatting rules:

1.  **Indentation**
* Use **4 spaces**，or **1 Tab**。
2.  **Braces**
* **Line break**: All curly braces `{` must be placed on a separate line.
3.  **Spacing**
* A space must follow control flow keywords `if`, `for`, `while`, `switch`, `catch`.
* *Good:* `if (condition)`
* ~~*Bad:* `if(condition)`~~
---
## Code Example
...
public class PlayerController : MonoBehaviour, IInteractable
{
    ...
    // Private static: s_ + PascalCase
    private static PlayerController s_Instance;

    // Public static: PascalCase
    public static PlayerController Instance => s_Instance;

    // SerializeField: m_ + PascalCase
    [SerializeField] private int m_Health = 100;
    
    // Protected: m_ + PascalCase
    ...
    private void Awake()
    {
        // Place a space after the keyword; break the line after the curly brace.
        if (s_Instance == null) 
        {
            s_Instance = this;
        }
...
Mainstream IDEs like JetBrains Rider and Visual Studio fully support managing coding styles through `.editorconfig` configuration files, eliminating repetitive editing across different projects or devices.

In your C# project's root directory (at the same level as the `.sln` file), create a new `NamingStyle.editorconfig` file and add the following content.
```ini
# editorconfig.org
...
---
## References
You can refer to JetBrains and Microsoft's detailed C#-related configurations in editorconfig.
